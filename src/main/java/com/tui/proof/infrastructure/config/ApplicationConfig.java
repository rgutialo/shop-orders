...
/**
 * Bean definitions for classes defined in the application package and needed to be managed by Spring framework
 */
@Configuration
public class ApplicationConfig {
    ...
    @Bean
    public OrderWriterPort orderWriterPort(IOrderRepo orderRepo, OrderMapper orderMapper, IAddressRepo addressRepo, IClientRepo clientRepo) {
    ...
    @Bean
    public AddressReaderPort addressReaderPort(IAddressRepo addressRepo, AddressMapper addressMapper) {
        return new AddressReaderAdapter(addressRepo, addressMapper);
    }

    @Bean
    public AddressWriterPort addressWriterPort(IAddressRepo addressRepo, AddressMapper addressMapper) {
        return new AddressWriterAdapter(addressRepo, addressMapper);
    }

    @Bean
    public ClientPort clientPort(ClientReaderPort clientReaderPort, ClientWriterPort clientWriterPort) {
        return new ClientService(clientReaderPort, clientWriterPort);
    }

    @Bean
    public AddressPort addressPort(AddressReaderPort addressReaderPort, AddressWriterPort addressWriterPort) {
        return new AddressService(addressReaderPort, addressWriterPort);
    }

    @Bean
    public OrderPort createOrderPort(OrderReaderPort orderReaderPort, OrderWriterPort orderWriterPort, ClientPort clientPort, AddressPort addressPort,
                                     PilotePriceReaderPort pilotePriceReaderPort) {
        return new OrderService(orderReaderPort, orderWriterPort, clientPort, addressPort, pilotePriceReaderPort);
    }
